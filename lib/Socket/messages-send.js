...
import { aggregateMessageKeysNotFromMe, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, DEF_MEDIA_HOST, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, MessageRetryCoordinator, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, unixTimestampSeconds, withMessageRetryInvalidation, delay } from '../Utils/index.js';
    ...
    const createParticipantNodes = async (recipientJids, message, extraAttrs, dsmMessage, alreadyPatched = false) => {
        ...
        const encryptionPromises = patchedMessages.map(async ({ recipientJid: jid, message: patchedMessage }) => {
            try {
                ...
                console.error('encrypt failed for', jid, err?.message, err?.stack?.split('\n')[1]);
                return null;
            }
        });
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                const relay = () => relayMessage(jid, fullMsg.message, {
					    messageId: fullMsg.key.id,
