...
export const makeMessagesRecvSocket = (config) => {
    ...
    const sendMessagesAgainPrepared = async (key, ids, retryNode, receiptNode, preparedRetries) => {
        const remoteJid = key.remoteJid;
        const participant = key.participant || remoteJid;
        const activeIds = []
		  const activePreparedRetries = []
        ...
        const retryableIds = []
		  const retryablePreparedRetries = []
            ...
            const preparedRetry = retryablePreparedRetries[i]
			   const resend = async () => {
                ...
                if (msg && (await willSendMessageAgain(id, participant))) {
					    ...
					    const msgRelayOpts = { messageId: id }
    ...
    const sendMessagesAgain = (key, ids, retryNode, receiptNode) => {
	    const remoteJid = key.remoteJid
